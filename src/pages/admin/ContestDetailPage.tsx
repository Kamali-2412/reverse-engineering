...
function AddProblemModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (initial) {
        console.log('[DEBUG] Updating problem', initial.id)
        await problemAPI.update(initial.id, { title, description, score })
        problemId = initial.id
      } else {
        console.log('[DEBUG] Creating problem', { contest_id: contestId, title })
        const res = await problemAPI.create({ contest_id: contestId, title, description, score })
        console.log('[DEBUG] Problem created', res.data)
        problemId = res.data.id
      }
      if (executableFile) {
        console.log('[DEBUG] Uploading executable', executableFile.name, executableFile.size)
        const uploadRes = await problemAPI.uploadExecutable(problemId, executableFile)
        console.log('[DEBUG] Executable uploaded', uploadRes.data)
      }
      if (testcaseFile) {
        console.log('[DEBUG] Uploading testcases', testcaseFile.name, testcaseFile.size)
        const tcRes = await testcaseAPI.uploadBulk(problemId, testcaseFile)
        console.log('[DEBUG] Testcases uploaded', tcRes.data)
      }
      ...
      console.error('[DEBUG] Save error response:', err?.response?.data)
...
export default function ContestDetailPage() {
  ...
  const handleUploadExec = async (problemId: string) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Failed to upload executable')
    }
  }
  ...
  const handleUploadTC = async (problemId: string) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Failed to upload test cases')
    }
  }
